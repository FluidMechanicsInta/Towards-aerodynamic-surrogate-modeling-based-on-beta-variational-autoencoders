...
# Working directories
general_outputs_folder = "./outputs/fine_tuning_VAE"
os.makedirs(name=general_outputs_folder, exist_ok=True)

logs_folder = f"./{general_outputs_folder}/01_logs_ft_VAE"
os.makedirs(name=logs_folder, exist_ok=True)

checkpoints_folder = f"./{general_outputs_folder}/02_checkpoints_ft_VAE"
os.makedirs(name=checkpoints_folder, exist_ok=True)

# Path to old checkpoint files
old_checkpoints_folder = "./outputs/training_VAE/02_checkpoints_VAE"
...
for beta in beta_sweep:
    ...
    for latent_dim in latent_dim_sweep:
        ...
        old_checkpoint_file = auxiliar.find_file(path_to_folder=old_checkpoints_folder, beta=beta,
                                                 latent_dimension=latent_dim, pca_bool=False)

        # Get old model
        old_model = lib_model.VAE(data_size=Cp_train_scaled.shape[1], latent_dim=latent_dim)
        ...
        gammas_train_ground_truth = gammas[f"beta{beta}_ls{2}_pca{False}"]["gammas_train"]
        gammas_test_ground_truth = gammas[f"beta{beta}_ls{2}_pca{False}"]["gammas_test"]
        ...
        # Set up dataset
        np_training_dataset = np.concatenate((predicted_train, Cp_train_scaled), axis=1)
        np_testing_dataset = np.concatenate((predicted_test, Cp_test_scaled), axis=1)
        ...
        model_name = (f"{str_date}_ft_net0_beta{beta}_dim{latent_dim}_lr{lr}OneCycleLR{lr_end}_bs{batch_size}_"
                      f"epochs{epochs}")
