...
# Working directories
general_outputs_folder = "./outputs/training_MLP"
os.makedirs(name=general_outputs_folder, exist_ok=True)

logs_folder = f"./{general_outputs_folder}/01_logs_MLP"
os.makedirs(name=logs_folder, exist_ok=True)

checkpoints_folder = f"./{general_outputs_folder}/02_checkpoints_MLP"
...
mlp = lib_model.MLP(data_size=Cp_train.shape[1]).to(device)
...
# Set up dataset
np_training_dataset = np.concatenate((MA_train, Cp_train_scaled), axis=1)
np_testing_dataset = np.concatenate((MA_test, Cp_test_scaled), axis=1)

# Set up dataloader
dataset_train = torch.utils.data.DataLoader(dataset=torch.from_numpy(np_training_dataset).to(torch.float32).cuda(0),
                                            batch_size=batch_size, shuffle=True, num_workers=0)

dataset_test = torch.utils.data.DataLoader(dataset=torch.from_numpy(np_testing_dataset).to(torch.float32).cuda(0),
                                           batch_size=batch_size, shuffle=False, num_workers=0)

# Train loop
str_date = time.strftime("%Y%m%d_%H_%M")

model_name = f"{str_date}_MLP_net0_lr{lr}OneCycleLR{lr_end}_bs{batch_size}_epochs{epochs}"
