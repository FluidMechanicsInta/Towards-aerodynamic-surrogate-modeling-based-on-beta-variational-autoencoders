...
# Working directories
general_outputs_folder = "./outputs/training_VAE_prePCA"
os.makedirs(name=general_outputs_folder, exist_ok=True)

logs_folder = f"./{general_outputs_folder}/01_logs_VAE_prePCA"
os.makedirs(name=logs_folder, exist_ok=True)

checkpoints_folder = f"./{general_outputs_folder}/02_checkpoints_VAE_prePCA"
...
dataset_train = torch.utils.data.DataLoader(dataset=torch.from_numpy(PCA_train_scaled).to(device),
                                            batch_size=batch_size, shuffle=True, num_workers=0)

dataset_test = torch.utils.data.DataLoader(dataset=torch.from_numpy(PCA_test_scaled).to(device),
...
for beta in beta_sweep:
    ...
    for latent_dim in latent_dim_sweep:
        ...
        # Get model
        model = lib_model.VAE_PrePCA(data_size=PCA_train_scaled.shape[1], latent_dim=latent_dim).to(device)
        ...
        model_name = (f"{str_date}_PCAnet0_beta{beta}_dim{latent_dim}_lr{lr}OneCycleLR{lr_end}_bs{batch_size}_"
                      f"epochs{epochs}")
